...
escMsg =
  let rex = fromRight' (\_ -> unsafeCrashWith "Invalid regex in escMsg") $ regex "^" $ Regex.parseFlags "gm"
    in Regex.replace rex "  "

-- create a TAP-safe error msg
escTitle :: String -> String
escTitle =
  let rex = fromRight' (\_ -> unsafeCrashWith "Invalid regex in escTitle") $ regex "#" $ Regex.parseFlags "g"
    in Regex.replace rex ""
